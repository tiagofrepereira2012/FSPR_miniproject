...
def pca_covmat(X):
  ...
  # 
  mu_X=X.mean(axis=1).reshape(X.shape[0],1);
  sigma=(numpy.dot((X-mu_X), (X-mu_X).T))/(X.shape[1]-1)
  e,U=numpy.linalg.eigh(sigma)
...
def pca_svd(X):
  ...
  # The shapes of e and U should be similar to the shapes below
  mu_X=X.mean(axis=1).reshape(X.shape[0],1);
  U, s, V =numpy.linalg.svd(X-mu_X, full_matrices=False)
  e=(s*s)/(X.shape[1]-1)
  #e = numpy.ones((X.shape[0],), dtype=float)
  #U = numpy.ones((X.shape[0], X.shape[0]), dtype=float)
